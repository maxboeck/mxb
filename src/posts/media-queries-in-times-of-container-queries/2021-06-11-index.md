...
The current implementation of `@container` only allows querying the width of an element (its "inline" axis), not its height. Container queries are still very experimental though, so this might still change down the road.
