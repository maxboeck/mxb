...
The underlying issue with both these methods is the same: external build processes. That's why some Eleventy setups are going a slightly different route: instead of running assets pipelines on the outside, they teach Eleventy itself to handle them. That way, everything runs through a single, integrated process. 
